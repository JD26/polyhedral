...
\section{Polyhedrals, polytopes and barycentric coordinates}

Let us start with some definitions and facts. Our reference is \cite{ABrIP1983}\href{file:references/ABrIP1983.pdf}{§}.

An affine subspace of $\mathbb{R}^{d}$ is either the empty set or
a translate of a linear subspace, that is, a subset $x+L$ where $x\in\mathbb{R}^{d}$
and $L$ is a linear subspace of $\mathbb{R}^{d}$. The dimension
of a non-empty affine subspace $A$ is the dimension of the linear
subspace $L$ such that $A=x+L$. For any subset $M$ of $\mathbb{R}^{d}$,
there is a smallest affine subspace containing $M$, namely, the intersection
of all affine subspaces containing $M$. This affine subspace is called
the affine hull of M, and it is denoted by $\mathrm{aff}\,$M.
...
$M$. This convex set is called the convex hull of M, and it is denoted
by $\mathrm{conv}\,$M. Let $C$ be a convex set of $\mathbb{R}^{d}$.
By the dimension of $C$ we mean the dimension of the affine hull
of $C$. A point $x\in C$ is called an extreme point of $C$ if $C\backslash\left\{ x\right\} $
is again convex. If $C$ is in addition compact, $C$ is the convex
hull of its extreme points.

A subset $Q$ of $\mathbb{R}^{d}$ is called polyhedral set if $Q$
is the intersection of a finite number of closed half-spaces or $Q=\mathbb{R}^{d}$.
Every polyhedral set is closed and convex. A polyehedral set $Q\neq\mathbb{R}^{d}$
has the form
\begin{equation}
Q=\bigcap_{k\in\mathcal{K}}\left\{ x\in\mathbb{R}^{d}\left|\,x\cdot n_{k}\leq a_{k}\right.\right\} ,\label{eq:rep}
\end{equation}
where $\mathcal{K}$ is a finite set of indices, $n_{k}\in\mathbb{R}^{d}\backslash\left\{ \overrightarrow{0}\right\} $
and $a_{k}\in\mathbb{R}$. This representation of $Q$ is called irreductible
if $\left|\mathcal{K}\right|=1$, or $\left|\mathcal{K}\right|>1$
and
\[
Q\subsetneq\bigcap_{k\in\mathcal{K}\backslash\left\{ j\right\} }\left\{ x\in\mathbb{R}^{d}\left|\,x\cdot n_{k}\leq a_{k}\right.\right\} \quad\forall\;j\in\mathcal{K}.
\]
Let $Q$ be a polyhedral set in $\mathbb{R}^{d}$ with dimension $d$,
$Q\neq\mathbb{R}^{d}$, and $\left|\mathcal{K}\right|>1$. Then the
representation (\ref{eq:rep}) is irreductible if and only if
\[
\left\{ x\in\mathbb{R}^{d}\left|\,x\cdot n_{j}=a_{j}\right.\right\} \cap\bigcap_{k\in\mathcal{K}\backslash\left\{ j\right\} }\left\{ x\in\mathbb{R}^{d}\left|\,x\cdot n_{k}\leq a_{k}\right.\right\} \neq\emptyset\quad\forall\;j\in\mathcal{K}.
\]
...
$S^{n-1}=\left\{ x\in\mathbb{R}^{n}\mid\left\Vert x\right\Vert =1\right\} $
is the unit $\left(n-1\right)$-sphere.

Let $\mathcal{K}$ be a finite set of indices.
\begin{defn}
For each $k\in\in\mathcal{K}$, let $\widehat{\phi}_{k}:\mathbb{R}^{n}\times\mathbb{R}\rightarrow\mathbb{R}$
be the function defined by
\[
\widehat{\phi}_{k}\left(x,t\right)\coloneqq x\cdot n_{k}\left(t\right)-a_{k}\left(t\right),
\]
where $n_{k}:\mathbb{R}\rightarrow S^{n-1}$ and $a_{k}:\mathbb{R}\rightarrow\mathbb{R}$
are arbitrary $C^{\infty}$ functions. For each $k\in\mathcal{K}$
and $t\in\mathbb{R}$, let $\omega_{k}\left(t\right)$ be the affine
set defined by
\[
\omega_{k}\left(t\right)\coloneqq\left\{ x\in\mathbb{R}^{n}\mid\widehat{\phi}_{k}\left(x,t\right)<0\right\} .
\]
...
\end{defn}
%
Note that $\omega_{k}\left(t\right)$ is one of the two open half-spaces
determined by the plane $\left\{ x\in\mathbb{R}^{3}\left|\,x\cdot n_{k}\left(t\right)=a_{k}\left(t\right)\right.\right\} $.
Denote $n_{k}\coloneqq n_{k}\left(0\right)$, $a_{k}\coloneqq a_{k}\left(0\right)$,
$\widehat{\phi}_{k}\left(\cdot\right)\coloneqq\widehat{\phi}_{k}\left(\cdot,0\right)$
$\omega_{k}\coloneqq\omega_{k}\left(0\right)$, $\Omega\coloneqq\Omega_{0}$.
Thus, as $\Omega=\bigcap_{k\in\mathcal{K}}\omega_{k}$ is a finite
intersection of open sets, $\Omega$ is a open set; as $\overline{\Omega}=\bigcap_{k\in\mathcal{K}}\overline{\omega_{k}}$
is a finite intersection of closed half-spaces, $\overline{\Omega}$
is a polyhedral set. 

Consider the following assumptions:
\begin{enumerate}
\item $a_{k}>0$ $\forall\;k\in\mathcal{K}$. This implies that $\overrightarrow{0}\in\Omega$.
Also, from this we can deduce that $\dim\overline{\Omega}=3$. Indeed,
since $\overrightarrow{0}$ is a interior point, there exists a point
$y\in\overline{\Omega}\backslash\left\{ \overrightarrow{0}\right\} $
in the line segment joining $x$ and $\overrightarrow{0}$. Then $x\in\mathrm{aff}\left\{ \overrightarrow{0},y\right\} \subset\mathrm{aff}\,\overline{\Omega}$.
Therefore $\mathrm{aff}\,\overline{\Omega}=\mathbb{R}^{3}$.
\item $\exists\;R>0$ such that $\overline{\Omega}\subset B\left(\overrightarrow{0},R\right)$.
That is, $\overline{\Omega}$ is bounded. Moreover $\overline{\Omega}$
is not empty. Therefore $\overline{\Omega}$ is a polytope and
\[
...
\item $\partial\omega_{j}\cap\mathrm{int}\bigcap_{k\in\mathcal{K}\backslash\left\{ j\right\} }\overline{\omega_{k}}\neq\emptyset$
...
\item Each $v$ is contained in precisely $3$ faces. As $\dim\overline{\Omega}=3$,
...
$z_{v}:\left(-\tau,\tau\right)\rightarrow\mathbb{R}^{3}$ satisfying
...
\end{defn}
\begin{thm}
There exist a smooth function $T:\overline{\Omega}\times\left(-\tau,\tau\right)\rightarrow\mathbb{R}^{3}$
such that $T\left(\cdot,t\right)=\partial\Omega$ and $T\left(\cdot,t\right)$
is bijective.
\end{thm}
%
\begin{proof}
